...
try {
    ...
    $check = $conn->query("SHOW TABLES LIKE 'detallepagoxcpbte'");
    if ($check->num_rows > 0) {
        echo "🔁 Tabla existente encontrada. Eliminando...\n";
        $conn->query("DROP TABLE detallepagoxcpbte");
    }

    // Crear tabla con índice nuevo incluido
    $sql = "
    CREATE TABLE `detallepagoxcpbte` (
        ...
        INDEX `idx_anio_nro_expte` (`anio`, `nro_comprobante`, `expediente`)
    )
    ...
    if ($conn->query($sql) === TRUE) {
        echo "✅ Tabla 'detallepagoxcpbte' creada con éxito";
    } else {
